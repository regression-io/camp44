...
def test_oidc_login_redirect(mock_oauth, enable_oauth):
    ...
    # Make request
    # We need to patch fastapi.Request.url_for which is called inside the oidc_login function
    with patch("fastapi.Request.url_for", return_value="/callback"):
        response = client.get("/api/v1/auth/oidc/login")
    
    # Verify redirect
    assert response.status_code == 302
    # No need to check the Location, the mock will handle that
...
def test_oidc_callback_existing_user(mock_user_crud, mock_create_token, mock_oauth, enable_oauth):
    ...
    mock_user = MagicMock()
    mock_user.id = uuid.uuid4()
    # Use regular MagicMock for sync method
