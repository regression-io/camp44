...
### Security
- **Migrate OIDC token validation off the deprecated `authlib.jose` → `joserfc`** (closes the two follow-ups flagged in the floors-pinning entry below). `camp44/core/oauth.py` `OIDCTokenValidator.validate_token` now decodes/verifies via `joserfc` (`jwt.decode(..., algorithms=["RS256"])` + `JWTClaimsRegistry` for `iss`/`aud`/`exp`) instead of `authlib.jose.JsonWebToken`/`JsonWebKey`, which 1.7 deprecates and 2.0 removes. **Behavior-preserving:** new contract tests (`tests/oidc/test_token_validation.py`, 7) — valid RS256 → claims; wrong `aud`/`iss`, expired, HS256-Camp44, and garbage tokens → `None` (never raise) — pass identically before and after the swap, and the `AuthlibDeprecationWarning` is gone. The HS256 log-suppression still works (joserfc's `UnsupportedAlgorithmError` message also contains `unsupported_algorithm`). `authlib` stays (its `starlette_client` OAuth login flow is **not** deprecated). New `joserfc>=1.0.0` dep. **Also closes the deferred passkey check:** `tests/passkey/test_attestation_cbor_decode.py` (2) drives a real `attestationObject` (COSE EC2 P-256 → `authData` → CBOR) through `py_webauthn`'s `parse_attestation_object` + `decode_credential_public_key` on **cbor2 6.1.1** and asserts clean decode — pinning the "does the attestation path survive cbor2 6.x" concern to ground truth. Full local suite: 23 passed (was 14; the 11 errors are the pre-existing Postgres/alembic harness issue, unrelated).
- **Pin the security floors that were only holding by drift** (follow-up to F5; closes its deferred `starlette`/`fastapi` item). The build has no lockfile and every dep was an open `>=`, so a fresh image resolves *latest* — which is why prod already runs **fastapi 0.136.3 / starlette 1.2.0 / authlib 1.7.2 / python-multipart 0.0.29** and `pip-audit` on a fresh resolve is **clean**. The "major starlette 0.x→1.x bump" was therefore already live and healthy, but **accidental**: a resolver picking an older fastapi would silently un-bump starlette below its CVE fix. Codified the floors in `dependencies` so the patched state can't regress — `fastapi>=0.135.0` (drops the `starlette<1.0` cap), `starlette>=1.0.1` (PYSEC-2026-161), `authlib>=1.6.11`, `python-multipart>=0.0.27`. Direct starlette use is only stable middleware imports (`BaseHTTPMiddleware`, `Request/Response`, `SessionMiddleware`, `ASGIApp`, `types`), unchanged across 1.0. **Fixed the dead cbor2 cap:** the F5 `[tool.uv] constraint-dependencies = ["cbor2<6"]` never took effect because both Dockerfiles install with **pip**, which ignores `[tool.uv]` — prod has run **cbor2 6.1.1** all along. Removed the misleading constraint and added an honest `cbor2>=5.9.0` (no upper cap) to `dependencies`; the WebAuthn passkey *registration* path verified working on 6.x. **Deferred (own follow-ups):** migrate `camp44/core/oauth.py` off the deprecated `authlib.jose`→`joserfc` before authlib 2.0 (works on 1.7.x, deprecation warning only); and a fuller passkey *attestation-verification* smoke on cbor2 6.x. *(Both done — see the `joserfc` entry above.)*
