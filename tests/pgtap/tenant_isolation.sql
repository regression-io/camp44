...
-- Create two tenants
SELECT gen_random_uuid() INTO TEMP TABLE t (id uuid);
INSERT INTO t VALUES (gen_random_uuid()), (gen_random_uuid());

\set tenant_a `SELECT id FROM t LIMIT 1`
\set tenant_b `SELECT id FROM t OFFSET 1 LIMIT 1`

-- Helper to set tenant_id
CREATE OR REPLACE FUNCTION _set_tenant(uuid) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
...
SELECT is(
    (SELECT count(*) FROM meter_event), 1,
    'tenant A sees 1 row'
);
...
SELECT is(
    (SELECT count(*) FROM meter_event), 0,
