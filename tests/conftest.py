...
from camp44.core.security import get_password_hash, create_access_token
...
def test_app(client: TestClient, db_session: Session) -> App:
    ...
    try:
        ...
        if cleanup_response.status_code != 200:
            ...
            db_session.execute(sa_text(f"DELETE FROM entity WHERE app_id = '{created_app['id']}'"))
            db_session.execute(sa_text(f"DELETE FROM app WHERE id = '{created_app['id']}'"))
            db_session.commit()
    except Exception:
        pass
...
def test_user(db_session: Session) -> User:
    ...
    if not user:
        raise ValueError("Test user not found. The client fixture creates the test user.")
    return user
